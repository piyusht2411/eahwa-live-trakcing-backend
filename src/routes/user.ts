...
import { deleteUser, getAdminsAndManagers, getAllUsers, getMyTeamUsers, getUserById, updateUser, getUsersHomeLocations, getUserTravelHistory, switchActiveMode } from "../controllers/userController";
import { protect, authorize } from "../middleware/auth";

const router = Router();
router.get("/", protect, authorize("admin", "super_manager", "hr", "manager"), getAllUsers);
router.get("/my-team", protect, authorize("admin", "super_manager", "hr", "manager"), getMyTeamUsers);
...
router.delete("/:id", protect, authorize("admin", "super_manager", "manager", "hr"), deleteUser);
